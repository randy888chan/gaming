...
import "./interfaces/IGambaToken.sol"; // Add Gamba token interface
 ...
 contract PolymarketAdapter is ZetaCommon, ZetaReceiver, Ownable {
     ...
     function onZetaMessage(
         ...
         // Handle Gamba token integration
         require(IERC20(gambaToken).transferFrom(msg.sender, address(this), amount),
             "Gamba token transfer failed");
         ...
         if (payload.actionType == 0) {
             ...
             conditionalTokens.splitPosition(
                 ...
                 bytes32(0),
                 payload.conditionId,
                 payload.dataArray, // Changed from payload.partition to payload.dataArray
                 amount
             );
         } else if (payload.actionType == 1) {
             // Merge Positions (Claim Winnings)
             collateral.approve(address(conditionalTokens), amount); // Added approval for mergePositions
             ...
             conditionalTokens.redeemPositions(
                 ...
                 bytes32(0),
                 payload.conditionId,
                 payload.dataArray // Changed from payload.indexSets to payload.dataArray
             );
         } else {
     ...
     /// @param targetChainId The chain ID of the target chain where the CrossChainSettlement contract is deployed.
     ...
     function placeBet(
         ...
         // Initiate the cross-chain swap via CrossChainSettlement
     ...
     function onZetaRevert(
         ...
         require(IERC20(gambaToken).transfer(context.zetaTxSenderAddress, amount),
             "Refund failed");
