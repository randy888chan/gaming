...
describe("LiquidityEngine", () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(particlePool, "deposit")
      .mockResolvedValue(mockTransactionReceipt);
    jest
      .spyOn(particlePool, "withdraw")
      .mockResolvedValue(mockTransactionReceipt);
  ...
  describe("deposit", () => {
    ...
    it("should throw an error if the pool is not found during deposit", async () => {
      ...
      const currency = "ETH";
      await expect(
        engine.deposit("nonExistent", amount, currency)
      ).rejects.toThrow("Liquidity pool nonExistent not found.");
    });
  });
  ...
  describe("withdraw", () => {
    ...
    it("should throw an error if the pool is not found during withdrawal", async () => {
      ...
      const currency = "USDC";
      await expect(
        engine.withdraw("nonExistent", amount, currency)
      ).rejects.toThrow("Liquidity pool nonExistent not found.");
    });
  });
  ...
  describe("getBalance", () => {
    ...
    it("should throw an error if the pool is not found during getBalance", async () => {
      ...
      const currency = "DAI";
      await expect(
        engine.getBalance("nonExistent", user, currency)
      ).rejects.toThrow("Liquidity pool nonExistent not found.");
    });
  });
});
...
describe("GambaLiquidityPool", () => {
  ...
  it("deposit should log and return a placeholder TransactionReceipt", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      `Depositing ${amount.toString()} ${currency} to Gamba pool`
    );
  ...
  it("withdraw should log and return a placeholder TransactionReceipt", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      `Withdrawing ${amount.toString()} ${currency} from Gamba pool`
    );
    expect(receipt).toEqual({}); // Expecting the placeholder
  });
  ...
  it("balanceOf should log and return a placeholder BigInt", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      `Checking balance of ${user} in Gamba pool for ${currency}`
    );
...
describe("ParticleLiquidityPool", () => {
  ...
  it("should initialize", () => {
    expect(console.log).toHaveBeenCalledWith(
      "ParticleLiquidityPool initialized"
    );
  });
  ...
  it("deposit should log and return a placeholder TransactionReceipt", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      `Depositing ${amount.toString()} ${currency} to Particle pool`
    );
    expect(receipt).toEqual({}); // Expecting the placeholder
  });
  ...
  it("balanceOf should log and return a placeholder BigInt", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      `Checking balance of ${user} in Particle pool for ${currency}`
    );
    expect(balance).toEqual(BigInt(0)); // Expecting the placeholder
  });
});
