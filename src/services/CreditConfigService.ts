...
class CreditConfigService {
  ...
  async getConfig(id: string): Promise<CreditConfig | null> {
    try {
      const stmt = this.db
        .prepare("SELECT * FROM credit_configs WHERE id = ?")
        .bind(id);
  ...
  async createConfig(
    data: Omit<CreditConfig, "id" | "created_at" | "updated_at">
  ...
  async updateConfig(
    id: string,
    updates: Partial<Omit<CreditConfig, "id" | "created_at" | "updated_at">>
    ...
    try {
      ...
      const stmt = this.db.prepare(query).bind(...bindings, id);
  ...
  async deleteConfig(id: string): Promise<boolean> {
    try {
      const stmt = this.db
        .prepare("DELETE FROM credit_configs WHERE id = ?")
        .bind(id);
