...
liquidityEngine.registerPool(
  "gamba",
  new GambaLiquidityPool("0xGambaContractAddress", {} as any)
); // Replace {} as any with actual provider
    ...
    case "POST":
      try {
        ...
        if (!name || !format || !status) {
          return res
            .status(400)
            .json({ error: "Missing required fields: name, format, status" });
        }
        ...
        if (isPolymarketTournament) {
          if (!entryFee || !entryCurrency) {
            return res
              ...
              .json({
                error:
                  "Entry fee and currency are required for Polymarket tournaments.",
              });
          }
          ...
          const POLYMARKET_ESCROW_CONTRACT_ADDRESS =
            "0xPolymarketEscrowContractAddress";
      ...
      try {
        ...
        if (!id || (!name && !format && !status && !triggerPayout)) {
          return res
            ...
            .json({
              error:
                "Missing required fields: id and at least one of name, format, status, or triggerPayout",
            });
        }
        ...
        if (success) {
          ...
          if (entryFee && entryCurrency && liquidityPoolName) {
            try {
              const amountBigNumber = BigInt(entryFee);
              await liquidityEngine.deposit(
                liquidityPoolName,
                amountBigNumber,
                entryCurrency
              );
              console.log(
                `Deposited ${entryFee} ${entryCurrency} to ${liquidityPoolName} for tournament ${id}`
              );
            } catch (liquidityError: any) {
              console.error(
                `Failed to deposit liquidity for tournament ${id}:`,
                liquidityError
              );
          ...
          if (
            ...
            try {
              ...
              await liquidityEngine.withdraw(
                sourcePoolName,
                amountBigNumber,
                payoutCurrency
              );
              ...
              if (payoutChain !== Chain.ZETACHAIN) {
                // Assuming ZetaChain is the native chain for internal operations
              ...
              console.log(
                `Processed payout of ${payoutAmount} ${payoutCurrency} to ${payoutRecipient} on ${payoutChain} for tournament ${id}`
              );
            } catch (payoutError: any) {
              console.error(
                `Failed to process payout for tournament ${id}:`,
                payoutError
              );
              return res
                .status(500)
                .json({
                  error: `Failed to process payout: ${payoutError.message}`,
                });
            }
          }
