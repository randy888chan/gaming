      ...
      try {
        ...
        if (id) {
          ...
          const { results } = await db
            .prepare("SELECT * FROM teams WHERE tournament_id = ?")
            .bind(tournament_id)
            .all();
    ...
    case "POST":
      try {
        ...
        if (!tournament_id || !name) {
          return res
            .status(400)
            .json({ error: "Missing required fields: tournament_id, name" });
        }
        const playersJson = players ? JSON.stringify(players) : null;
        const { success } = await db
          .prepare(
            "INSERT INTO teams (tournament_id, name, players) VALUES (?, ?, ?)"
          )
          .bind(tournament_id, name, playersJson)
          .run();
        if (success) {
      ...
      try {
        ...
        if (!id || (!name && !players)) {
          return res
            ...
            .json({
              error:
                "Missing required fields: id and at least one of name, players",
            });
        }
