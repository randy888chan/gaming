...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!amount || !targetChain || !privateKey) {
        return res.status(400).json({ message: 'Missing required parameters: amount, targetChain, privateKey' });
    }
    ...
    try {
        ...
        const swapId = await settlementService.initiateSwap(amountBigNumber, targetChain);

        res.status(200).json({ success: true, swapId });
    } catch (error: any) {
        console.error('Error initiating swap:', error);
        res.status(500).json({ message: 'Failed to initiate swap', error: error.message });
    }
}