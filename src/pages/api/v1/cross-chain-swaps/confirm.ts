...
export default async function handler(
  ...
  if (!swapId || !privateKey) {
    return res
      .status(400)
      .json({ message: "Missing required parameters: swapId, privateKey" });
  }
  ...
  try {
    ...
    await settlementService.confirmSwap(swapId);

    res
      .status(200)
      .json({ success: true, message: `Swap ${swapId} confirmed.` });
  } catch (error: any) {
    console.error("Error confirming swap:", error);
    res
      .status(500)
      .json({ message: "Failed to confirm swap", error: error.message });
  }
}
