...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      res.status(200).json({ message: 'Daily pSEO content generation triggered successfully.' });
    } catch (error) {
      console.error('Error triggering daily pSEO content generation:', error);
      res.status(500).json({ message: 'Failed to trigger daily pSEO content generation.' });
    }
  } else {
