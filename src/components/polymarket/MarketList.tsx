...
const MarketList: React.FC = () => {
  const [marketsData, setMarketsData] =
    useState<PaginatedSimplifiedMarkets | null>(null);
  ...
  const loadMarkets = useCallback(async (currentCursor: string | null) => {
    ...
    try {
      ...
      const activeOpenMarkets = data.data.filter(
        (market) => market.active && !market.closed
      );

      setMarketsData(data); // Store the full pagination response
      setVisibleMarkets((prevMarkets) =>
        currentCursor === ""
          ? activeOpenMarkets
          : [...prevMarkets, ...activeOpenMarkets]
      );
  ...
  if (visibleMarkets.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        <p className="text-xl">
          No active markets found on Polymarket at the moment.
        </p>
        <Button onClick={() => loadMarkets("")} className="mt-4">
          Refresh
        </Button>
        ...
        {visibleMarkets.map((market) => (
          <Card key={market.condition_id} className="flex flex-col">
            ...
            <CardContent className="flex-grow">
              ...
              <p className="text-sm text-gray-500 mb-1">
                Volume (USD): ${market.volume.toLocaleString()}
              </p>
              <p className="text-sm text-gray-500 mb-3">
                Liquidity (USD): ${market.liquidity.toLocaleString()}
              </p>
...
const SmartBetButton: React.FC<SmartBetButtonProps> = ({ market }) => {
  ...
  const handleSmartBetSuggestion = async () => {
    ...
    try {
      const response = await fetch(
        `/api/smart-bet?marketId=${market.condition_id}&userId=${user.id}`
      );
      const data: SmartBetSuggestion | { message: string } =
        await response.json();

      if (response.ok) {
        const suggestion = data as SmartBetSuggestion;
        toast.success(`Smart Bet Suggestion for ${suggestion.marketId}:`, {
          description: `Outcome: ${
            suggestion.outcome
          }, Amount: $${suggestion.suggestedBetAmount.toFixed(
            2
        ...
        toast.error(
          `Failed to get smart bet suggestion: ${
      ...
      toast.error(
        "An unexpected error occurred while fetching smart bet suggestion."
      );
    } finally {
  ...
  const handlePlaceBet = async (outcomeIndex: number, amount: number) => {
    ...
    try {
      ...
      toast.info(
        "Betting functionality requires a direct `placeBet` function in the smart contract."
      );
      toast.success(
        `Simulated bet of $${amount.toFixed(
          2
        )} on outcome ${outcomeIndex} for market ${market.question}`
      );
    } catch (error) {
  ...
  return (
    <div className="flex flex-col space-y-2">
      <Button
        onClick={handleSmartBetSuggestion}
        disabled={loadingSuggestion}
        className="w-full"
      >
        {loadingSuggestion
          ? "Getting Suggestion..."
          : "Get Smart Bet Suggestion"}
      </Button>
