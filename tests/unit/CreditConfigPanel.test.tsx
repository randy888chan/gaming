...
describe("CreditConfigPanel", () => {
  beforeEach(() => {
    ...
    global.fetch = jest.fn((url: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (
        url.toString().startsWith("/api/v1/admin/credit-config") &&
        ...
        url.toString() === "/api/v1/admin/credit-config" &&
        init?.method === "PUT"
      ) {
        // Changed POST to PUT
        ...
        if (
          body.rules.creditAmountUSDC <= 0 ||
          !body.rules.treasuryWallet ||
          body.rules.chains.length === 0 ||
          !body.rules.kmsProvider
        ) {
          return Promise.resolve(
            new Response(
              JSON.stringify({
        ...
        return Promise.resolve(
          new Response(JSON.stringify({ success: true, config: body }), {
            status: 200,
          })
        );
      }
  ...
  test("renders and loads initial config", async () => {
    ...
    expect(screen.getByLabelText("Chains (comma-separated):")).toHaveValue(
      "solana, ethereum"
    );
  ...
  test("shows validation errors for invalid input on save", async () => {
    ...
    const creditAmountInput = screen.getByLabelText("Credit Amount (USDC):");
    fireEvent.change(creditAmountInput, {
      target: { name: "creditAmountUSDC", value: "0" },
    });

    const treasuryWalletInput = screen.getByLabelText("Treasury Wallet:");
    fireEvent.change(treasuryWalletInput, {
      target: { name: "treasuryWallet", value: "" },
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Credit Amount (USDC) must be a positive number.")
      ).toBeInTheDocument();
      expect(
        screen.getByText("Treasury Wallet cannot be empty.")
      ).toBeInTheDocument();
      expect(
        screen.getByText("At least one chain must be specified.")
      ).toBeInTheDocument();
      expect(
        screen.getByText("KMS Provider cannot be empty.")
      ).toBeInTheDocument();
    });
  });
  ...
  test("saves configuration successfully", async () => {
    ...
    const creditAmountInput = screen.getByLabelText("Credit Amount (USDC):");
    fireEvent.change(creditAmountInput, {
      target: { name: "creditAmountUSDC", value: "150" },
    });
  ...
  test("displays error message on fetch failure", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Error: Failed to fetch config")
      ).toBeInTheDocument()
    );
  });
  ...
  test("displays error message on save failure", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Error: Failed to save config")
      ).toBeInTheDocument();
    });
  });
});
