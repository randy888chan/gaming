...
jest.mock("gamba-react-v2", () => ({
  useGamba: jest.fn(() => ({
    ...
    play: jest.fn(),
    result: jest.fn(() =>
      Promise.resolve({ payout: 0, multiplier: 0, token: "SOL" })
    ),
  })),
}));
...
jest.mock("gamba-react-ui-v2", () => ({
  ...
  EffectTest: ({ src }: { src: string }) => (
    <div
      data-testid="effect-test"
      style={{ backgroundImage: `url(${src})` }}
    ></div>
  ), // Moved EffectTest to top level
...
jest.mock("../../src/games/Slots/Slot", () => ({
  Slot: ({
    ...
    <div
      data-testid={`slot-${
        item?.image?.split("/").pop()?.split(".")[0] || Math.random()
      }`}
...
jest.mock("../../src/games/Slots/utils", () => {
  ...
  return {
    generateBetArray: jest.fn(() => {
      console.log(
        "[TEST MOCK] generateBetArray actually called, returning [2,0,0]"
      );
    ...
    getSlotCombination: jest.fn((numSlots, multiplier, bet) => {
      ...
      if (multiplier > 0) {
        const winningItem =
          SLOT_ITEMS.find((item) => item.multiplier === multiplier) ||
          SLOT_ITEMS[0] ||
          mockSlotItemsFallback[0];
        return Array(numSlots).fill(winningItem);
      }
      return Array(numSlots).fill(
        SLOT_ITEMS.find((item) => item.multiplier === 0) ||
          mockSlotItemsFallback[1]
      );
    }),
  };
}); // Single semicolon
...
describe("Slots Game Component Integration Tests", () => {
  ...
  test("simulates a winning spin", async () => {
    ...
    (useGamba as jest.Mock).mockReturnValue({
      ...
      result: jest.fn(() =>
        Promise.resolve({ payout: 100, multiplier: 10, token: "SOL" })
      ), // Simulate win via useGamba().result
    });
    ...
    jest
      .mocked(require("../../src/games/Slots/utils").getSlotCombination)
      .mockReturnValue(Array(3).fill(SLOT_ITEMS[0])); // NUM_SLOTS is 3
  ...
  test("simulates a losing spin", async () => {
    ...
    (useGamba as jest.Mock).mockReturnValue({
      ...
      result: jest.fn(() =>
        Promise.resolve({ payout: 0, multiplier: 0, token: "SOL" })
      ), // Simulate lose via useGamba().result
    });
    // The GambaUi.useGame().play mock is still needed if the component calls game.play()
    (GambaUi.useGame as jest.Mock).mockReturnValue({
      play: mockPlayGame,
    ...
    const losingCombination = [
      SLOT_ITEMS.find((item) => item.multiplier === 0) ||
        SLOT_ITEMS[SLOT_ITEMS.length - 1], // Wojak or last item as fallback
      SLOT_ITEMS.find(
        (item) =>
          item.multiplier === 0 &&
          item.id !== SLOT_ITEMS[SLOT_ITEMS.length - 1].id
      ) || SLOT_ITEMS[SLOT_ITEMS.length - 2], // Another 0 multiplier item, different from first
      SLOT_ITEMS.find(
        (item) =>
          item.multiplier === 0 &&
          item.id !== SLOT_ITEMS[SLOT_ITEMS.length - 1].id &&
          item.id !== SLOT_ITEMS[SLOT_ITEMS.length - 2].id
      ) || SLOT_ITEMS[SLOT_ITEMS.length - 3], // Yet another different 0 multiplier item
    ...
    while (losingCombination.length < 3 && losingCombination.length > 0)
      losingCombination.push(losingCombination[losingCombination.length - 1]);
    if (losingCombination.length === 0) {
    ...
    jest
      .mocked(require("../../src/games/Slots/utils").getSlotCombination)
      .mockReturnValue(losingCombination.slice(0, 3));
