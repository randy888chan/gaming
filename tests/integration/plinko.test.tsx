...
jest.mock('gamba-react-ui-v2', () => {
  ...
  return {
    GambaUi: {
      ...
      Canvas: ({ render }: { render: (args: { ctx: CanvasRenderingContext2D, size: { width: number, height: number } }, clock: any) => void }) => {
      ...
      Switch: ({ checked, onChange, disabled }: { checked: boolean, onChange: (checked: boolean) => void, disabled?: boolean }) => (
        <input type="checkbox" data-testid="plinko-switch" checked={checked} onChange={(e) => onChange(e.target.checked)} disabled={disabled} />
      ),
    },
...
describe('Plinko Game Component Integration Tests', () => {
  ...
  test('simulates placing a bet and winning', async () => {
    ...
    await waitFor(() => {
      expect(mockPlay).toHaveBeenCalledWith({ wager: 1, bet: expect.any(Array) });
    });
